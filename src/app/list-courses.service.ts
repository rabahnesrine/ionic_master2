...
/* getCoursesById(id):Course{
  return this.listCourses[id];
} */

getCoursesById(id) {
  return this.listCourses.find((cours)=>(cours.id==id));
}

addCourses(c:Course){
 

  let id = this.listCourses.length + 1 ;
  c.id=id;

  console.log("id:"+c.id);
 // this.listCourses.unshift();
 this.listCourses.push(c) ;
}
